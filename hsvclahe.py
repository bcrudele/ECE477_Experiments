...
# Apply CLAHE to the grayscale version of the image
gray_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
clahe = cv2.createCLAHE(clipLimit=5)
clahe_image = clahe.apply(gray_image)

# Create a mask that isolates the orange color from the original image
...
# Show the images
cv2.imshow("CLAHE Image", clahe_image)  # Grayscale with CLAHE applied
...
# Save the results
cv2.imwrite("clahe_image2.jpg", clahe_image)
cv2.imwrite("orange_isolated2.jpg", result)
